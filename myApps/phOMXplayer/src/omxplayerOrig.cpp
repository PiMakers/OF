...
void phOMXplayer::SetVideoMode(int width, int height, int fpsrate, int fpsscale, FORMAT_3D_T is3d)
{
  int32_t num_modes = 0;
  int i;
  ...
  score += (1<<16) + (1<<20)/r; // bad - but prefer higher framerate
      ...
      float par = rpiUtils->get_display_aspect_ratio((HDMI_ASPECT_T)tv->aspect_ratio)*(float)tv->height/(float)tv->width;
  ...
  if(tv_found)
  {
    ...
    m_BcmHost.vc_tv_register_callback(rpiUtils->CallbackTvServiceCallback, &tv_synced);
    int success = m_BcmHost.vc_tv_hdmi_power_on_explicit_new(HDMI_MODE_HDMI, (HDMI_RES_GROUP_T)group, tv_found->code);
    if (success == 0)
      sem_wait(&tv_synced);
    m_BcmHost.vc_tv_unregister_callback(rpiUtils->CallbackTvServiceCallback);
...
int phOMXplayer::omxPlayerCli (char* str) {

    char** newargs;
    newargs[0] = "omxPlayerCli2";
    newargs[1] = str;
    int argc=2;

    if ( omxplayerOrig ( argc, newargs))
        {
            ofLogError(__func__);
            return 1;
        }
...
int phOMXplayer::omxplayerOrig( int argc, char** argv)
{
/*
...
*/
  ...
  bool filename_is_URL = pUtils->IsURL(m_filename);

  if(!filename_is_URL && !pUtils->IsPipe(m_filename) && !pUtils->Exists(m_filename))
  {
    PrintFileNotFound(m_filename);
    return EXIT_FAILURE;
  }

  if(m_asked_for_font && !pUtils->Exists(m_font_path))
  {
    PrintFileNotFound(m_font_path);
    return EXIT_FAILURE;
  }

  if(m_asked_for_italic_font && !pUtils->Exists(m_italic_font_path))
  {
    PrintFileNotFound(m_italic_font_path);
    return EXIT_FAILURE;
  }

  if(m_has_external_subtitles && !pUtils->Exists(m_external_subtitles_path))
  {
  ...
  if(!m_has_external_subtitles && !filename_is_URL)
  {
    ...
    if(pUtils->Exists(subtitles_path))
    {
  ...
  int gpu_mem = rpiUtils->get_mem_gpu();
  int min_gpu_mem = 64;
  ...
  if (false == m_no_keys)
  {
    pUtils->m_keyboard = new Keyboard();
  }
  if (NULL != pUtils->m_keyboard)
  {
    pUtils->m_keyboard->setKeymap(keymap);
    pUtils->m_keyboard->setDbusName(m_dbus_name);
  }
  ...
  if(current_tv_state.state & ( VC_HDMI_HDMI | VC_HDMI_DVI )) {
    //HDMI or DVI on
    m_config_video.display_aspect = rpiUtils->get_display_aspect_ratio((HDMI_ASPECT_T)current_tv_state.display.hdmi.aspect_ratio);
  } else {
    //composite on
    m_config_video.display_aspect = rpiUtils->get_display_aspect_ratio((SDTV_ASPECT_T)current_tv_state.display.sdtv.display_options.aspect);
  }
    ...
    if (m_last_check_time == 0.0 || m_last_check_time + DVD_MSEC_TO_TIME(20) <= now)
    {
     ...
     if (update) {
       OMXControlResult result = control_err
                               ? (OMXControlResult)(pUtils->m_keyboard ? pUtils->m_keyboard->getEvent() : KeyConfig::ACTION_BLANK)
    ...
    switch(result.getKey())
    {
     case KeyConfig::ACTION_CHANGE_FILE:
      ...
      case KeyConfig::ACTION_EXIT:
        m_stop = true;
        do_exit();
        ofLogNotice(__func__) << "case KeyConfig::ACTION_EXIT:";
        break;
...
int phOMXplayer::do_exit() {
  ...
  /// exit status success on playback end
    if (m_send_eos) {
        ofLogNotice(__func__) << "VIDEO_ENDED!!!";
        return EXIT_SUCCESS;
    }

  /// exit status OMXPlayer defined value on user quit
    if (m_stop) {
        ofLogNotice(__func__) << "VIDEO_STOPPED!!!";
        return 3;
    }
  /// exit status failure on other cases
  ofLogNotice(__func__) << "EXIT_FAILURE!!!";
  return EXIT_FAILURE;
}
